...
- On **Latest**, articles sit in a **centred reading column** (`--col`, ~768px): the headline, body, and the dividing rules share that width and align — never a full-width headline over a narrow paragraph. Body text is **18px** (lead **20px**), line-height ~1.6, giving an ~80-character measure (the readability sweet spot; do not stretch wider). The indicators strip and the POC flagship stay full-width. Stories are a **ruled column, not boxes**. The **first story in each section is the lead**: bigger grotesk headline + a vertical **accent bar** on its left (`.story-card:first-child::after`) — NOT a drop cap.
